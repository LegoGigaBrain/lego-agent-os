# Skill: Business Strategy

## Purpose

Develop comprehensive business strategies including market positioning, competitive analysis, business models, and strategic planning. This skill provides frameworks for strategic decision-making.

## Standards Awareness

Before applying this skill, ensure familiarity with:
- `standards/signalos/transmission-framework.md`
- `standards/signalos/brand-strategy.md`

---

## Core Philosophy

> "Strategy is making choices about where to play and how to win."

Business strategy is:
- Focused choices, not comprehensive plans
- Trade-offs, not trying to do everything
- Sustainable advantage, not short-term wins
- Aligned with brand signal and values

---

## Strategy Frameworks

### 1. Playing to Win

**Where to Play:**
- Which markets/segments?
- Which geographies?
- Which channels?
- Which customer types?

**How to Win:**
- Cost leadership?
- Differentiation?
- Niche focus?
- What capabilities enable this?

```
STRATEGY CHOICE CASCADE:

1. WINNING ASPIRATION:
   What does winning look like?

2. WHERE TO PLAY:
   - Market: [Choice]
   - Segment: [Choice]
   - Geography: [Choice]
   - Channel: [Choice]

3. HOW TO WIN:
   - Competitive advantage: [Choice]
   - Differentiation: [Choice]

4. CAPABILITIES REQUIRED:
   - [Capability 1]
   - [Capability 2]

5. MANAGEMENT SYSTEMS:
   - [System to enable strategy]
```

### 2. SWOT Analysis

```
SWOT ANALYSIS:

STRENGTHS (Internal Positive):
- [Strength 1]
- [Strength 2]

WEAKNESSES (Internal Negative):
- [Weakness 1]
- [Weakness 2]

OPPORTUNITIES (External Positive):
- [Opportunity 1]
- [Opportunity 2]

THREATS (External Negative):
- [Threat 1]
- [Threat 2]

STRATEGIC IMPLICATIONS:
- Leverage: [How to use strengths for opportunities]
- Defend: [How to use strengths against threats]
- Improve: [How to address weaknesses]
- Avoid: [What to stay away from]
```

### 3. Porter's Five Forces

```
FIVE FORCES ANALYSIS:

COMPETITIVE RIVALRY:
- Number of competitors: [Assessment]
- Industry growth: [Assessment]
- Exit barriers: [Assessment]
- Overall force: [High/Medium/Low]

THREAT OF NEW ENTRANTS:
- Barriers to entry: [Assessment]
- Capital requirements: [Assessment]
- Overall force: [High/Medium/Low]

BARGAINING POWER OF SUPPLIERS:
- Supplier concentration: [Assessment]
- Switching costs: [Assessment]
- Overall force: [High/Medium/Low]

BARGAINING POWER OF BUYERS:
- Buyer concentration: [Assessment]
- Price sensitivity: [Assessment]
- Overall force: [High/Medium/Low]

THREAT OF SUBSTITUTES:
- Availability: [Assessment]
- Switching costs: [Assessment]
- Overall force: [High/Medium/Low]

INDUSTRY ATTRACTIVENESS: [Overall assessment]
```

### 4. Value Proposition Canvas

```
CUSTOMER PROFILE:

JOBS TO BE DONE:
- Functional: [What they need to accomplish]
- Social: [How they want to be perceived]
- Emotional: [How they want to feel]

PAINS:
- [Pain 1]
- [Pain 2]

GAINS:
- [Gain 1]
- [Gain 2]

---

VALUE MAP:

PRODUCTS/SERVICES:
- [What we offer]

PAIN RELIEVERS:
- [How we address pains]

GAIN CREATORS:
- [How we create gains]

FIT ASSESSMENT:
[How well value map matches customer profile]
```

### 5. Business Model Canvas

```
BUSINESS MODEL:

KEY PARTNERS:
[Who we need]

KEY ACTIVITIES:
[What we do]

KEY RESOURCES:
[What we need]

VALUE PROPOSITION:
[What we offer]

CUSTOMER RELATIONSHIPS:
[How we relate]

CHANNELS:
[How we reach them]

CUSTOMER SEGMENTS:
[Who we serve]

COST STRUCTURE:
[What it costs]

REVENUE STREAMS:
[How we make money]
```

---

## Strategic Planning Process

### Step 1: Situation Analysis

```
CURRENT STATE:
- Market position: [Where we are]
- Financial health: [Key metrics]
- Competitive position: [Vs. competitors]
- Capabilities: [What we're good at]

EXTERNAL ENVIRONMENT:
- Market trends: [What's changing]
- Competitive dynamics: [What competitors are doing]
- Regulatory: [What affects us]
- Technology: [What's emerging]
```

### Step 2: Strategic Options

```
STRATEGIC OPTIONS:

OPTION 1: [Name]
- Description: [What it involves]
- Pros: [Advantages]
- Cons: [Disadvantages]
- Requirements: [What's needed]
- Risk level: [Assessment]

OPTION 2: [Name]
[Same structure]

OPTION 3: [Name]
[Same structure]
```

### Step 3: Strategy Selection

```
EVALUATION CRITERIA:
- Strategic fit: [Weight]
- Financial impact: [Weight]
- Feasibility: [Weight]
- Risk level: [Weight]

OPTION SCORING:
| Criterion | Weight | Option 1 | Option 2 | Option 3 |
|-----------|--------|----------|----------|----------|
| [Criterion]| [%] | [Score] | [Score] | [Score] |

RECOMMENDED STRATEGY:
[Selected option with rationale]
```

### Step 4: Strategic Roadmap

```
STRATEGIC ROADMAP:

PHASE 1: [Timeframe]
Objectives:
- [Objective]
Key initiatives:
- [Initiative]
Success metrics:
- [Metric]

PHASE 2: [Timeframe]
[Same structure]

PHASE 3: [Timeframe]
[Same structure]
```

---

## Output Templates

### Strategy One-Pager

```
# Strategy Summary: [Company/Initiative]

## Winning Aspiration
[What success looks like]

## Where to Play
- Market: [Choice]
- Segment: [Choice]
- Geography: [Choice]

## How to Win
[Competitive advantage]

## Key Initiatives
1. [Initiative]
2. [Initiative]
3. [Initiative]

## Success Metrics
- [Metric]: [Target]
```

### Strategic Plan

```
# Strategic Plan: [Period]

## Executive Summary
[Overview]

## Situation Analysis
[Current state and environment]

## Strategic Direction
[Where to play and how to win]

## Strategic Priorities
[Key focus areas]

## Roadmap
[Phased plan]

## Resources Required
[Investment needed]

## Risks & Mitigation
[Key risks and responses]

## Governance
[How we manage execution]
```

---

## Quality Checklist

- [ ] Strategy involves clear choices
- [ ] Trade-offs are explicit
- [ ] Advantage is sustainable
- [ ] Strategy is aligned with brand
- [ ] Roadmap is realistic
- [ ] Metrics are defined
- [ ] Risks are addressed

---

## Anti-Patterns

- Trying to do everything
- No clear trade-offs
- Copying competitors
- Strategy without execution plan
- Ignoring market reality
- Misaligned with brand
