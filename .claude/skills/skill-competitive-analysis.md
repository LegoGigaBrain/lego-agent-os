# Skill: Competitive Analysis

## Purpose

Conduct comprehensive competitive analysis to understand market positioning, identify opportunities, and inform strategic decisions. This skill provides frameworks for competitive intelligence.

## Standards Awareness

Before applying this skill, ensure familiarity with:
- `standards/signalos/brand-strategy.md`

---

## Core Philosophy

> "Know your competition, but compete with yourself."

Competitive analysis is:
- Understanding the landscape, not copying
- Finding whitespace, not following
- Learning from others' mistakes
- Informing differentiation strategy

---

## Competitive Analysis Framework

### 1. Competitor Identification

```
COMPETITOR MAPPING:

DIRECT COMPETITORS:
(Same product, same market)
- [Competitor 1]
- [Competitor 2]
- [Competitor 3]

INDIRECT COMPETITORS:
(Different product, same need)
- [Competitor 1]
- [Competitor 2]

POTENTIAL COMPETITORS:
(Could enter the space)
- [Competitor 1]
- [Competitor 2]

SUBSTITUTES:
(Alternative solutions)
- [Alternative 1]
- [Alternative 2]
```

### 2. Competitor Profile

For each key competitor:

```
COMPETITOR PROFILE: [Name]

OVERVIEW:
- Founded: [Year]
- Size: [Employees/Revenue]
- Funding: [If relevant]
- Headquarters: [Location]

PRODUCT/SERVICE:
- Core offering: [What they sell]
- Key features: [Main capabilities]
- Pricing: [Model and levels]
- Target market: [Who they serve]

POSITIONING:
- Value proposition: [Their promise]
- Messaging: [Key messages]
- Brand personality: [How they present]
- Differentiation claim: [What makes them unique]

STRENGTHS:
- [Strength 1]
- [Strength 2]

WEAKNESSES:
- [Weakness 1]
- [Weakness 2]

RECENT MOVES:
- [Strategic action 1]
- [Strategic action 2]
```

### 3. Competitive Comparison

```
FEATURE COMPARISON:

| Feature | Us | Comp A | Comp B | Comp C |
|---------|-----|--------|--------|--------|
| [Feature 1] | ✓/✗ | ✓/✗ | ✓/✗ | ✓/✗ |
| [Feature 2] | ✓/✗ | ✓/✗ | ✓/✗ | ✓/✗ |

POSITIONING MAP:
[2x2 matrix with relevant axes]

Axis 1: [Dimension] Low ←→ High
Axis 2: [Dimension] Low ←→ High

- Us: [Position]
- Comp A: [Position]
- Comp B: [Position]

PRICING COMPARISON:
| Tier | Us | Comp A | Comp B |
|------|-----|--------|--------|
| [Tier] | [$] | [$] | [$] |
```

### 4. Messaging Analysis

```
MESSAGING COMPARISON:

COMPETITOR: [Name]

TAGLINE: [Their tagline]
HEADLINE: [Main website headline]
VALUE PROP: [Their stated value]

KEY MESSAGES:
1. [Message 1]
2. [Message 2]
3. [Message 3]

PROOF POINTS:
- [How they prove claims]

TONE/VOICE:
- [Description of communication style]

DIFFERENTIATION CLAIM:
- [What they say makes them unique]
```

### 5. Visual/Brand Analysis

```
BRAND ANALYSIS: [Competitor]

VISUAL IDENTITY:
- Logo style: [Description]
- Color palette: [Colors]
- Typography: [Style]
- Imagery: [Approach]

BRAND PERSONALITY:
- Archetype: [Apparent archetype]
- Tone: [Communication style]

BRAND STRENGTHS:
- [What works]

BRAND WEAKNESSES:
- [Where they fall short]
```

---

## Strategic Insights

### Whitespace Identification

```
WHITESPACE ANALYSIS:

POSITIONING GAPS:
[Where competitors aren't playing]

FEATURE GAPS:
[Unmet needs in the market]

AUDIENCE GAPS:
[Underserved segments]

MESSAGING GAPS:
[Stories not being told]

OPPORTUNITY ASSESSMENT:
| Gap | Size | Fit | Priority |
|-----|------|-----|----------|
| [Gap] | [H/M/L] | [H/M/L] | [Score] |
```

### Competitive Advantages

```
SUSTAINABLE ADVANTAGE ASSESSMENT:

OUR ADVANTAGES:
- [Advantage 1]: [How sustainable]
- [Advantage 2]: [How sustainable]

COMPETITOR ADVANTAGES:
- [Competitor]: [Their advantage]
- [Competitor]: [Their advantage]

DEFENSIBILITY:
[How we protect our position]
```

### Battlecard Creation

```
BATTLECARD: vs. [Competitor]

WHEN THEY COME UP:
[Common situations]

THEIR PITCH:
[What they likely say]

OUR COUNTER:
[How we respond]

WHERE WE WIN:
- [Scenario 1]
- [Scenario 2]

WHERE THEY WIN:
- [Scenario 1]
- [Scenario 2]

KEY PROOF POINTS:
- [Evidence 1]
- [Evidence 2]

OBJECTION HANDLING:
"[Objection]"
→ "[Response]"
```

---

## Output Templates

### Competitive Landscape Report

```
# Competitive Landscape: [Market]

## Executive Summary
[Key findings]

## Market Overview
[Market context]

## Competitor Profiles
[Summary of key competitors]

## Comparative Analysis
[Feature, pricing, positioning comparisons]

## Whitespace & Opportunities
[Where to differentiate]

## Strategic Recommendations
[What to do with this intel]

## Appendix
[Detailed competitor profiles]
```

### Quick Competitive Brief

```
# Competitive Brief: [Competitor]

## Who They Are
[Overview]

## What They Offer
[Product/service]

## How They Position
[Messaging and differentiation]

## Strengths
[Where they're strong]

## Weaknesses
[Where they fall short]

## How We Compete
[Our approach against them]
```

---

## Quality Checklist

- [ ] Key competitors identified
- [ ] Profiles are current
- [ ] Comparison is fair and accurate
- [ ] Whitespace is identified
- [ ] Insights are actionable
- [ ] Battlecards are useful

---

## Anti-Patterns

- Analysis paralysis
- Competitor obsession
- Biased assessment
- Outdated intelligence
- Copying instead of differentiating
- Ignoring indirect competition
